...
export function calcSizing(monthlyKwh, peakSunHours, roofAreaSqFt, monthlyIrradiance = []) {
  ...
  // Required system size to cover 100% of usage
  const systemSizeKW = monthlyKwh / (30 * peakSunHours * systemEfficiency)
  ...
  const coveragePercent = Math.min(100, Math.round((monthlyGenKwh / monthlyKwh) * 100))
...
export function calcFinancials(sizing, options = { applySubsidy: true }) {
  ...
  const rawInstallCost = sizing.systemSizeKW * costPerKW
  ...
  const paybackYears = parseFloat((installCost / annualSavings).toFixed(1))
  ...
  for (let yr = 1; yr <= systemLifespan; yr++) {
    ...
    yearlyData.push({
      ...
      cumulativeCost: Math.round(installCost + annualMaintenance * yr),
    })
  }
